...
class Function(ecosim.base.Storable):
    ...
    def __init__(self,yields,sources):
        ...
        if len(yields) != len(sources):
            raise ValueError(f"Lengths of yields and sources do not coincide ({len(yields)} != {len(sources)}).")
...
class InteractionNetwork(ecosim.base.Storable):
    ...
    def __init__(self,S):
        ...
        self._S = S
        self._random_component = ecosim.glv.RandomLotkaVolterra.InteractionModel(None, {}, None)
    ...
    def add_function(self,function):
        if function.S != self._S:
            raise ValueError(f"Number of species in Function does not match number of species in InteractionNetwork ({function.S} != {self._S})")
    ...
    def set_random_component(self,name,**kwargs):
        ...
        if name in ecosim.glv.RandomLotkaVolterra._available_interaction_models:
            int_mod = ecosim.glv.RandomLotkaVolterra.InteractionModel(name,
                ecosim.glv.RandomLotkaVolterra._available_interaction_models_defaults[name],
    ...
    def sample_matrix(self,include_functions=True,include_direct_interactions=True,seed=None):
        if self._random_component is not None:
            self._last_random_sample = self._random_component.sampler(self.S, **self.random_component_params,seed=seed)
...
class DynamicModel(ecosim.base.Storable):
    ...
    def __init__(self,S):
        ...
        self._interaction_network = InteractionNetwork(S) #it is important that this is the first created object, otherwise __setattr__ will fail
    ...
    def __setattr__(self, name, value):
        ...
        if name != '_interaction_network' and name in self._interaction_network._random_component.params:
    ...
    def interaction_network(self):
        raise RuntimeError("In place access to InteractionNetwork is not implemented. Use the wrapper functions of the DynamicModel class to modify it instead.")
