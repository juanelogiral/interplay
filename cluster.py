...
class InteractionClusteringModel(ecosim.base.Storable,metaclass=icm_metaclass):
    ...
    def get_method_params(self):
        if not (self._clustering_method,self._optimization_method) in InteractionClusteringModel._available_clustering_functions:
            raise ValueError(f"The pair {(self._clustering_method,self._optimization_method)} does not match any clustering function.")
        
        code_alias = InteractionClusteringModel._available_clustering_functions[(self.clustering_method,self.optimization_method)].__code__
    ...
    def __call__(self, interaction_network,n_clusters,**kwargs):
        ...
        cluster_sp_map, sp_cluster_map, accuracy, info_dict = InteractionClusteringModel._available_clustering_functions[(self._clustering_method,self._optimization_method)](interaction_network,n_clusters,**kwargs)
        ...
        for c1,c2 in self:
            new_mat[np._ix(c1,c2)] = old_mat[np._ix(c1,c2)] @ np.ones(len(c1),len(c2))/len(c2)
